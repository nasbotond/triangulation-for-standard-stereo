...
int main()
{
    for (int i = 1; i < 6; i++)
    {
        ...
        // Move origin for each dataset to (0, 0) using the u and v parameters from the K matrix
        ...
        */
        ...
        double b = 120.0; // mm (12 cm), baseline
        double f = 3.8; // mm, focal length
        ...
        {
            // convert from pixel to mm by dividing by pixel/mm value
            u1 = points_img1[i].x / pixel;
            u2 = points_img2[i].x / pixel;
            v1 = points_img1[i].y / pixel;
            d = u1 - u2; //disparity

            // spatial pooints converted to m from mm
            newSpatialPoint.z = (b * f) / d / 1000.0;
            newSpatialPoint.x = (-b * (u1 + u2)) / (2.0 * d) / 1000.0;
            newSpatialPoint.y = (b * v1) / d / 1000.0;

            // constrain to points less than 500 meters away from origin
